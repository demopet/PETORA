...
const PetDetailTabs: React.FC<PetDetailTabsProps> = ({ active, onChange }) => {
  const tabs = [
...
export default function PetDetailPage() {
  ...
  const ownerName =
    customers?.find((c) => c.id === pet.customer_id)?.name || "Unknown";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          {pet.gender && (
            <p className="mt-1 text-sm text-slate-500">Gender: {pet.gender}</p>
          )}
        </div>
      ...
      <div className="space-y-6 rounded-lg bg-white p-6 shadow-sm">
        <PetDetailTabs active={activeTab} onChange={setActiveTab} />

        <div className="pt-6">
          {activeTab === "overview" && (
            <PetOverviewTab petId={pet.id} pet={pet} />
          )}
