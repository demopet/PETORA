import { Bell, CalendarDays, Gift, PawPrint, ShoppingBag, UserCircle } from 'lucide-react'
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-5 text-slate-900">
      ...
      <section className="mb-6 rounded-2xl border border-primary-100 bg-gradient-to-r from-primary-50 to-white p-4 shadow-sm">
        ...
        <div className="mt-3 flex items-center justify-between gap-4">
          ...
          <button type="button" className="rounded-full bg-primary-600 px-4 py-2 text-sm font-medium text-white">
            Lihat detail
          </button>
        </div>
      </section>
      ...
      <section className="mb-6">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {quickActions.map(({ label, icon: Icon }) => (
            <button key={label} type="button" className="rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm">
      ...
      <section className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
            ...
            <p className="mt-1 text-sm text-emerald-600">💉 Vaksin: Semua up-to-date</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-medium text-primary-700">2,450 poin</span>
        </div>
        <p className="mt-2 text-sm text-slate-600">Poin tersedia untuk ditukarkan dengan layanan berikutnya.</p>
      </section>
    </div>
  )
}
